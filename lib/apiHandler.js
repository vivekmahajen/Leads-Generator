...
function describeError(err) {
  ...
  if (
    ...
    /^P1\d\d\d$/.test(code) ||
    /Environment variable not found|POSTGRES_PRISMA_URL|DATABASE_URL/i.test(msg) ||
    ...
    return {
      status: 503,
      message:
        'Database is not configured or unreachable. Check the POSTGRES_PRISMA_URL connection setting, ensure the database is running, and that migrations have been applied.',
    };
  }
