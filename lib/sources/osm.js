...
// Lightweight total count for a category+location (Overpass `out count;`).
// Used by the marketing SEO pages — one geocode + one count query, cached.
export async function countLocalBusinesses(categoryId, location) {
  ...
  const query = `[out:json][timeout:25];(${lines.join('')});out count;`;
  ...
  if (!res.ok) {
    const e = new Error(`Count lookup failed (${res.status}).`);
    e.status = 502;
    throw e;
  }
  const data = await res.json();
  const total = Number(data.elements?.[0]?.tags?.total || 0);
  return { total, location: box.display, state: box.regionState };
}
