...
export async function findLocalBusinesses(categoryId, location, limit = 120) {
  ...
  for (const el of data.elements || []) {
    ...
    seen.add(key);
    scanned += 1;
    // Only keep businesses that list a contact email — those are the ones you
    // can actually reach. (Most OSM listings have a website but no email.)
    if (c.email) contacts.push(c);
  }
  return { contacts, scanned, location: box.display };
}
