...
/**
 * Lightweight count of discharge-planning providers in a state (+ optional city).
 * Used by the programmatic SEO pages, which only need a real headline number.
 * De-dupes across taxonomies so the count matches what a search would deliver.
 */
export async function countNpiProviders({ state, city, categoryId }) {
  const { found } = await findNpiProviders({ state, city, categoryId, limit: 200 });
  return { total: found, state: state || null, city: city || null };
}
