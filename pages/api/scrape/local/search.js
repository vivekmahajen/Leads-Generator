...
async function handler(req, res) {
  ...
  try {
    const { contacts, scanned, listed, guessed, location: resolved } = await findLocalBusinesses(category_id, String(location).trim());
    ...
    return res.status(200).json({ withEmail: contacts.length, scanned, listed, guessed, ...results, location: resolved });
  } catch (err) {
